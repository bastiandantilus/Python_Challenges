...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    mu = [[],[]]
    for xy in range(2):
        Omega[xy] = matrix([[1.0]]).expand(size, size, [0])
        #Omega[xy].show('Omega')
        
        Xi[xy] = matrix([[50.0]]).expand(size, 1, [0])
        #Xi[xy].show("Xi")

        OmegaDelta = matrix([[1, -1],[-1, 1]])
        
  
        for d in range(len(data)):
            observations, motion = data[d]
            #print (motion)
            Omega[xy] += OmegaDelta.expand(size, size, [d, d+1])
            XiDelta = matrix( [ [ -motion[xy] ] , [ motion[xy] ] ] )
            Xi[xy] += XiDelta.expand(size, 1, [d, d+1], [0])
            #print (observations)
            for landmark in observations:
                i, x, y = landmark
                coord = [x, y]
                Omega[xy] += OmegaDelta.expand(size, size, [d, i+N])
                Xi[xy] += matrix([[-coord[xy]],[coord[xy]]]).expand(size, 1, [d, i+N], [0])
        Omega[xy].show()
        Xi[xy].show()
            
        mu[xy] = Omega[xy].inverse() * Xi[xy]
    res = []
    for value in range(len(mu[0].value)):
        res.append(mu[0].value[value])
        res.append(mu[1].value[value])
    mu = matrix( res )
