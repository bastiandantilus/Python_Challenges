...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    lanes = len(road)
    length = len(road[0])
    h = [[0 for j in range(length)] for i in range(lanes)]
        ...
        for i in range(lanes-1, -1, -1):
            if j == goal[1]:
                ...
                if road[i][j] == 0:
                    ...
                    h_local = (1 / road[i][j])
                test.append( h[i][j+1] + h_local)
                if i < lanes - 1:
                    test.append( h[i+1][j+1] + h_local + lane_change_cost)
                if i > 0:
                    test.append( h[i-1][j+1] + h_local + lane_change_cost)
                h[i][j] = min(test)  
    cost = h[init[0]][init[1]]
    return cost
...
silly_value = sum([sum([0. for ss in range(1000)]) for s in range(10000)])
unit_end = time.clock()
unit_time = unit_end-unit_start
stopwatch_start = time.clock()

# For comparable results, you might want to comment out the print statements that drag down the speed signifficantly
[solution_check(testing_suite) for s in range(10000)]

stopwatch_end = time.clock()
print ("Unit time: %.4f seconds" % (unit_time))
print ("Testsuite time: %.4f seconds" % (stopwatch_end-stopwatch_start))
print ("Benchmark: %.4f units" % ((stopwatch_end-stopwatch_start)/unit_time))
"""
