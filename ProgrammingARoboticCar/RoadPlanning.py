...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    lanes = len(road)
    length = len(road[0])
    if [lanes-1, length-1] != goal: # Sanity check vs problem description
        print ("Goal not at end")
        lanes, length = goal[0] - 1, goal[1] - 1
    costs = []
    path = []
    h = [[] for x in range(lanes)]
    print( lanes, length, goal)
    for i in range(init[1] + 1, length ):
        options = [1 / max(x[i], 1e-16) for x in road]
        geuristic = [0 for x in road]
        speeds = [x[i] for x in road]
        for o in range(lanes):
            if (lanes > length - i and o < lanes - (length - i)) or \
                options[o] == 0 or options[o] > 999 or abs(o - lane) > 1:
                options[o] = 999
            geuristic[o] = options[o] + o
            if speeds[o] == max(speeds):
                geuristic[o] -= o
        for hh in range(lanes):
            if geuristic[hh] == min(geuristic):
                p = hh
            h[hh].append(geuristic[hh])
        costs.append(options[p])
        path.append([p,i])
        if p != lane:
            lane = p
            costs[i-1] += lane_change_cost
  
    cost = sum(costs)
    #print( [format(v, ".3f") for v in costs + [cost]])
    print (init, path, goal)
    for i in road:
        print ([format(v, "03n") for v in i])
    #for hh in h:
    #    print ([format(v, "08.3f") for v in hh])
    return cost
...
def solution_check(test, epsilon = 0.00001):
    ...
    for i in range(len(answer_list)):
        if answer_list[i] == 1:
            ...
            print( 'Test case', i+1, 'failed. ', answer_list[i])
