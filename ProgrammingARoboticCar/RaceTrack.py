    ...
    def cte(self, radius):
        ...
        if self.x < radius:
            if self.y < radius:
                ...
                cte = orient - (((1.0 - (self.y / radius)) * (pi / 2.0)) + pi / 2.0)
            else:
                direction =("Turning right!")
                cte = orient - ((1.0 - (self.y - radius) / radius) * (pi / 2.0))
        elif self.x > radius and self.x < 3.0 * radius:
            if self.y > radius:
                direction =("Going right!")
                cte = (self.y - radius * 2.0)
            else:
                direction =("Going left!")
                cte =  .8*(0 - self.y) * .2*(pi - orient)
        else:
            if self.y > radius:
                direction =("Turning down!")
                cte = (orient + (2*pi)) - (((((self.y - radius) / radius) * (pi / 2.0)) + 1.5 * pi) % (2 * pi))
            else:
                direction =("Turning left!")
                cte = orient - (((self.y / radius) * (pi / 2.0)) + pi)
         
        return [cte, direction]
...
def run(params, radius, printflag = False):
    ...
    crosstrack_error, dir = myrobot.cte(radius) # You need to define the cte function!
    for i in range(N*2):
        diff_crosstrack_error = - crosstrack_error
        crosstrack_error, dir = myrobot.cte(radius)
        diff_crosstrack_error += crosstrack_error
