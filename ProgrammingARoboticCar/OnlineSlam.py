...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    expansion = [0, 1] + [i for i in range(4, dim + 2)]
    contraction = [i for i in range(2, dim + 2)]
    ...
    for k in range(len(data)):
        ...
        Omega = Omega.expand(dim+2, dim+2, expansion)
        Xi = Xi.expand(dim+2, 1, expansion, [0])
        ...
        for b in range(2):
            ...
            Xi.value[n+b  ][0]        += -motion[b] / motion_noise
            Xi.value[n+b+2][0]        +=  motion[b] / motion_noise
        A = Omega.take([0, 1], contraction)
        Bi = Omega.take([0,1]).inverse()
        C = Xi.take([0,1], [0])
        Omega = Omega.take(contraction) - (A.transpose() * Bi) * A
        Xi = Xi.take(contraction, [0]) - (A.transpose() * Bi) * C
        #Omega.show('LiveOmega')
        #Xi.show('LiveXi  ')
    # compute best estimate
    mu = Omega.inverse() * Xi
...
answer_mu = result[0].take([-2, -1], [0])
answer_omega = matrix(result[1].value)
...
solution_check(result, answer_mu, answer_omega)
# -----------
# Test Case 1
